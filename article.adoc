...
Out performance client is not closing out connections.
We update the test client code to force response objects to close their streams. We could deduce this by reviewing the test code - each client thread creates one WebClient, which it reuses for each call, so the remaining connection was likely common from response object input stream - closing that immediately reduced lingering connections.
